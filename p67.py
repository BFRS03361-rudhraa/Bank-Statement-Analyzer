...
def ocr_with_gemini(image_path,canonical_header=None):
    ...
    if canonical_header:
        ...
        Ensure that every transaction entry uses these exact column names as keys."""
    ...
    If a cell is empty in the document, leave it empty in JSON.
...
def ocr_with_gemini_with_fallback(image_path, canonical_header=None):
    ...
    if not parsed.get('transactions') and not parsed.get('metadata'):
        ...
        top_parsed = parse_json_from_llm(top_text) or {"metadata": {}, "transactions": []}
        bottom_parsed = parse_json_from_llm(bottom_text) or {"metadata": {}, "transactions": []}
        ...
        return {"metadata": combined_metadata, "transactions": combined_transactions}
...
def _extract_largest_json_object(text: str):
